...
- [x] 2026-09-25 — Header menu consistency (`SiteHeader.tsx`): all menu items now share one pill style (identical padding) with a shared hover zoom (`hover:scale-[1.05]`, 150 ms) matching the "Get a quote" highlight; header logo zooms subtly on hover (`scale-[1.04]`). Fixed the root cause of Home/About looking different: NavLink matching ignores hashes, so on the home page both Home (`/`) and About (`/#about`) rendered the active pill at once. Now activation is hash-aware (`homeActive` / `servicesActive` / `aboutActive` / `contactActive`) so exactly one item highlights per location; Services also lights up on `/services/*` and `#services`, Contact on `/contact` and `#quote`. The "Shop · Soon" placeholder keeps the pill shape but no hover effect (non-functional placeholder). Desktop + mobile panels updated.
