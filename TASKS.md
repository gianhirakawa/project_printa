...
- [x] 2026-09-25 — Header menu uniformity (`SiteHeader.tsx`, per Gian's feedback): ALL menu items (Home → Shop, desktop + mobile panels) now permanently carry the same circular-pill treatment: `rounded-full`, identical padding, `bg-white/10`, red inset shadow (`inset_0_-3px_0_brand-bright`) — previously only the "current" item had it, which made Home/About look padded while the others looked flat. (First pass used flat-bottom sticker rounding `999px 999px 6px 6px` — rejected as ugly; final shape fully circular.) All items share the hover zoom (`scale-[1.05]`, 150 ms) matching the "Get a quote" highlight — Shop included, it uses the exact same pill class but stays non-navigating. Header logo zooms subtly on hover (`scale-[1.04]`). Active-state logic removed (no longer needed — all items look the same).
- [x] 2026-09-25 — Mobile menu rework (`SiteHeader.tsx`, per Gian's feedback): Services is no longer a pre-expanded indented list. It's now the same full-width pill as Home/About/Contact/Shop (all mobile rows made uniform full-width), with a chevron; tapping it opens/closes a smooth accordion (`grid-template-rows` transition) revealing the 4 selectable service links using the same treatment as the desktop dropdown (title + description, hover/active `bg-ink-3` via `NavLink` active state, left red accent border for hierarchy). Accordion collapses on navigation, on reopening via the hamburger, and resets state on close. Chevron rotates + turns gold when open; `aria-expanded`/`aria-haspopup` on the toggle. Desktop hover dropdown untouched.
...
### 2026-09-25 (session 4) — Mobile menu: Services as uniform collapsible item ✅

- Mobile panel: all rows (Home, Services, About, Contact, Shop, Get a quote) are now uniform full-width pills. Services is a tap-to-expand accordion instead of an always-expanded sub-list (Gian didn't like the services appearing pre-collapsed/indented while other items were flat pills). Accordion reveals the 4 service links with the same `dropdownLink` treatment as the desktop dropdown (shortTitle + shortDescription, hover `bg-ink-3`, `NavLink` active highlight), inset with a 2 px red left border. Open state animates via `grid-rows-[0fr]→[1fr]`; chevron rotates and turns gold; `aria-expanded`/`aria-haspopup` set. State resets on navigation and when the panel is closed/reopened via the hamburger. Verified: `tsc -b` + `vite build` clean. No content, copy, or routing changes.
